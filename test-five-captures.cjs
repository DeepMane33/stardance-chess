...
async function testFiveCaptures() {
  const browser = await chromium.launch({ headless: false });
  const page = await browser.newPage();
  
  page.on('console', msg => {
    if (msg.type() === 'error' || msg.text().includes('TEST') || msg.text().includes('RESULT') || msg.text().includes('Capture')) {
  ...
  console.log('--- TESTING 5 CAPTURES ---');
  ...
  const result = await page.evaluate(async () => {
    ...
    // Play a sequence that generates multiple captures
    const whiteMoves = ['d4', 'e4', 'd5', 'e5', 'Nc3', 'Nxe5', 'Nxd7', 'Bxd7', 'Bxg7', 'Bxh8'];
    ...
    for (let i = 0; i < whiteMoves.length; i++) {
      ...
      if (m) {
        ...
        if (m.captured) captures.push({ move, captured: m.captured, result });
        ...
        console.log(`[TEST] Move ${move} not available, skipping`);
      }
    }
  ...
  await page.screenshot({ path: 'five-captures-test.png', fullPage: true });
...
testFiveCaptures().catch(console.error);